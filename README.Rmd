...
sora eliminates all of it. `sora()` writes data into shared memory
...
- Transparent IPC. All atomic vector types (via ALTREAL, ALTINTEGER,
  ...
  (via ALTLIST) work with `serialize()` and `mirai()` — no descriptor
  or attach step.
- Zero dependencies. Pure C against OS APIs (POSIX SHM, Win32 file
  mappings) — no Rcpp, no Boost, no C++ on the build path.
- Single SHM region per compound object. A 100-column data frame is
  one `mmap`, not 100.
